...
> Lição `les:n5-conectando-07` · tópico `top:n5-conectando` · **needs_review** (Layer C, aguarda professor).

**Objetivos:**
- Listar de forma não exaustiva com o molde ～たり～たりする ('fazer coisas como A, B, entre outras')
...
- Reconheço o kanji 火 (fogo) e sei que aparece em 火曜日 (terça-feira).
- Leio sem trocar 四日 (よっか) e 八日 (ようか).

---

## Exercícios
### 1. (recognition) {'pt-BR': 'O que a estrutura ～たり～たりする comunica numa frase como 読んだり休んだりします？'}
- **Resposta:** `{"choices": ["Uma lista COMPLETA de tudo o que se fez", "Uma lista de algumas ações representativas, entre outras", "Uma ordem obrigatória de ações, uma após a outra"], "correct": "Uma lista de algumas ações representativas, entre outras"}`
- {'pt-BR': "～たり～たりする lista exemplos sem fechar a lista: 'fazer coisas como A e B (entre outras)'. Não é uma lista exaustiva nem uma sequência fixa."}


### 2. (cloze) {'pt-BR': 'Complete com a estrutura de lista. 日曜日は本を読＿休んだりします。 (No domingo leio livros, descanso etc.)'}
- **Resposta:** `{"text": "んだり", "full": "日曜日は本を読んだり休んだりします"}`
- {'pt-BR': 'Do passado casual 読んだ tira-se 読んだり. O たり precisa aparecer em CADA item (読んだり e 休んだり), e a frase fecha com します.'}


### 3. (recognition) {'pt-BR': '読み方（よみかた）quer dizer o quê?'}
- **Resposta:** `{"choices": ["a maneira de ler / como se lê", "o livro de leitura", "ler em voz alta"], "correct": "a maneira de ler / como se lê"}`
- {'pt-BR': "方（かた）gruda na raiz do verbo (読み, de 読む) e cria o substantivo 'o modo de'. Logo 読み方 = 'a maneira de ler'. O kanji 読 (ler) está bem ali no começo."}


### 4. (sentence_build) {'pt-BR': "Monte 'Qual é a leitura deste kanji?' com as peças: [読み方] [何] [か] [この] [は] [漢字] [の] [です]"}
- **Resposta:** `{"order": ["この", "漢字", "の", "読み方", "は", "何", "です", "か"], "text": "この漢字の読み方は何ですか。"}`
- {'pt-BR': "A ideia central é 読み方 (a maneira de ler) como substantivo: この漢字の読み方 = 'a leitura deste kanji'. Encaixe 何 ('o quê') antes de です para perguntar qual é essa leitura."}


### 5. (production) {'pt-BR': "Diga em japonês 'Aos sábados eu faço coisas como ler livros e descansar.' usando 本 (livro), 読む (ler), 休む (descansar) na estrutura ～たり～たりする (forma educada no fim)."}
- **Resposta:** `{"text": "土曜日は本を読んだり休んだりします。", "accept": ["土曜日は本を読んだり休んだりします", "土曜日は本を読んだり休んだりします。", "どようびは本を読んだり休んだりします", "土曜日は本をよんだりやすんだりします", "本を読んだり休んだりします", "本を読んだり休んだりします。"]}`
- {'pt-BR': "Cada ação ganha たり (読んだり, 休んだり) e a frase fecha com します, que carrega tempo e polidez. Não pare em 休む: a marca da lista 'entre outras' é justamente o たり no último item antes de する/します."}
