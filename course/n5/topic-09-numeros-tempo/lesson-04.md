...
> Lição `les:n5-numeros-tempo-04` · tópico `top:n5-numeros-tempo` · **needs_review** (Layer C, aguarda professor).

**Objetivos:**
- Anexar os contadores 個, 冊, 回, 階 e 歳 aos números já estudados
- Reconhecer as mudanças de som mais comuns na contagem (como いっこ, さんがい, はっさい)
- Distinguir tamanho médio (中) de grande (大) ao descrever objetos

**Introduz:** gramática [—] · vocabulário [中, 個, 冊, 回, 歳, 階] · kanji [大] · kana [—]
...
- 階 conta andares de um prédio.
- 歳 conta idade ("... anos de idade").

> **[l1-advantage]**
A boa notícia: o contador vem sempre depois do número, na mesma ordem que você já usa em português ("três " + "vezes"). Você não precisa reorganizar a frase: é só colar o sufixo no número.
...
#### 回: quantas vezes
Quando você quer dizer "uma vez", "duas vezes", "de novo", o contador é 回:一回(uma vez),二回(duas vezes),三回(três vezes). Repare que 一回 também sofre o mesmo "engasgo".

#### 階: andares
Para andares de um prédio, use 階. Cuidado com este:三階(terceiro andar) muda o som de かい para がい. Os demais seguem o padrão:一階(térreo / 1º andar),二階(2º andar).

> **[culture]**
No Japão, o 一階 é o nível da rua (o nosso "térreo"), e 二階 é o primeiro andar de cima. A contagem começa no chão, sem o "térreo" separado que usamos no Brasil.
...
> **[tip]**
O kanji 大 significa "grande". Imagine uma pessoa de braços e pernas abertos ao máximo, ocupando o maior espaço possível: é exatamente esse o desenho. Você o encontra em 大(tamanho grande) e em muitas palavras de "grandeza".

#### Juntando tudo
O molde é sempre o mesmo: número + contador. Veja como uma mesma ideia muda só de sufixo:
- りんご三個= três maçãs (coisas).
- 本三冊= três livros.
- 三回= três vezes.
- 三階= terceiro andar.
- 三歳= três anos de idade.

> **[l1-pitfall]**
Não traduza "vezes" e "anos" como palavras soltas. Em japonês eles são sufixos colados no número, não palavras separadas. "Três vezes" é uma peça só:三回, não "san + uma palavra para vezes".
...
- Conto andares com 階 (かい), lembrando de さんがい.
- Digo a idade com 歳 (さい), e sei que 20 anos é はたち.
- Reconheço 大 (grande) e 中 (médio) em tamanhos.
