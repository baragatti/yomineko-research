# Substituição e comparação: にかわって, に比べて

> Lição `les:n3-perspectiva-03` · tópico `top:n3-perspectiva` · **needs_review** (Layer C, aguarda professor).

**Objetivos:**
- Dizer que alguém age no lugar de outra pessoa com にかわって
- Não confundir にかわって (em vez de) com に変わって (transformar-se em)
- Comparar duas coisas com に比べて, colocando o ponto de referência antes, ao contrário do português
- Saber que に比べて é neutro e que より é a opção mais casual

**Introduz:** gramática [n3-ni-kawatte, n3-ni-kurabete] · vocabulário [エンジン, 大いに, 大家, 応じる, 援助, 横断, 演奏, 演技, 演説, 王, 王子, 王様, 終える, 老い, 覆う, 追いつく, 追う] · kanji [件 任 信 側 利 在 組] · kana [—]

**Frases (por ID, do banco dissecado):** `sent:tatoeba-11013866`

---

### Substituição e comparação: にかわって, に比べて
Para fechar o tema, duas expressões de relação entre elementos. n3-ni-kawatte diz que alguém age no lugar de outra pessoa; n3-ni-kurabete compara duas coisas pondo a diferença em evidência. Ambas exigem atenção à ordem dos elementos, que costuma confundir quem fala português.

#### にかわって: no lugar de / em nome de
A expressão n3-ni-kawatte diz que alguém ou algo age no lugar de, em nome de ou em substituição a outra pessoa. A ideia central é a troca de quem realiza a ação.
Molde: [pessoa] にかわって [ação]
彼にかわって会議に出席します = Vou à reunião no lugar dele. Antes de um substantivo, usa-se にかわる; em texto formal aparece に代わり.

> **[warning]**
Existe um に変わって de pronúncia idêntica (do verbo 変わる, mudar) que significa transformar-se em, sentido totalmente diferente de em vez de. Repare que a diferença está só no kanji: にかわって (no lugar de) escreve-se に代わって, com o kanji 代, enquanto に変わって (transformar-se em) traz o kanji 変. Para troca de algo material (em troca de), prefira と引き換えに.

#### に比べて: comparado com / em comparação a
A expressão n3-ni-kurabete compara duas situações destacando a diferença. O termo que recebe に比べて é o ponto de referência da comparação; o resto da frase diz como o outro elemento difere dele. Registro neutro a levemente formal; na fala bem casual prefira より.
Molde: [referência] に比べて [item comparado] は [como difere]
夏に比べて冬は寒い = Comparado com o verão, o inverno é frio.

> **[l1-pitfall]**
Armadilha PT: o ponto de referência (o do que da comparação) leva に比べて, ordem inversa à do português, em que a referência vem depois (mais frio do que o verão). Em 夏に比べて冬は寒い, o verão é a base, e a frase diz que o inverno é frio em relação a ele. Para modificar um substantivo, use に比べた.

#### Kanji do dia: 利, 組, 信, 在, 件, 側, 任, 引
- 利 (lucro, vantagem, benefício). Arroz à esquerda, faca à direita: colher o grão é colher o ganho.
- 組 (grupo, conjunto, montar). O radical de fio à esquerda: fios trançados formam um grupo.
- 信 (confiança, fé). Pessoa à esquerda, palavra à direita: alguém fiel à sua palavra.
- 在 (existir, estar localizado). Terra na base: aquilo que está fincado num lugar.
- 件 (assunto, caso, item). Pessoa ao lado de boi: cada caso contado, item por item.
- 側 (lado, margem). Pessoa à esquerda: a pessoa que fica ao lado, a margem.
- 任 (responsabilidade, dever, confiar). Pessoa à esquerda: aquele a quem se confia uma tarefa.
- 引 (puxar, atrair, citar). Um arco com uma linha: puxar a corda do arco. Aparece em 引退 (aposentadoria, da lição 1).

#### Vocabulário desta lição
Comece pelas palavras de palco, ação e realeza. Repare que vários nomes começam com o som えん, escrito com o kanji 演 (apresentar-se, executar), e que outros giram em torno de 王 (rei).
- 演技: atuação
- 演説: discurso
- 演奏: execução musical
- 援助: auxílio
- エンジン: motor
- 横断: travessia
- 王: rei
- 王様: rei (forma com tratamento honorífico)
- 王子: príncipe
- 大家: locador, dono do imóvel
Em seguida, alguns verbos e substantivos do cotidiano. Muitos também começam com o som お, o que ajuda a guardá-los em bloco.
- 追う: perseguir
- 追いつく: alcançar
- 応じる: responder, atender
- 終える: terminar
- 覆う: cobrir
- 大いに: muito, bastante
- 老い: velhice
- 御: prefixo honorífico

> **[example]**
Combine substituição e comparação:王にかわって王子が演説をする (no lugar do rei, o príncipe faz o discurso);去年に比べて演技がうまい (comparado ao ano passado, a atuação está boa).

#### Exemplos do banco
> 🗣 先週に比べてペース上がりまくった。 — Comparado com a semana passada, o ritmo disparou pra valer.

#### Hora de praticar

#### Leitura
> 📖 明日の会議は、社長にかわって私が話します。社長は外国のおきゃくさんと会うことになりました。その日は、私にかわって、となりの人が電話に出ます。今年は去年にくらべて、仕事がとても多くなりました。前にくらべて時間が少ないので、みなさん早く来てください。 — Na reunião de amanhã, eu falo no lugar do presidente. Ficou decidido que ele vai atender um cliente de fora do país. Naquele dia, quem atende o telefone no meu lugar é a pessoa da mesa ao lado. Comparado ao ano passado, este ano o trabalho aumentou bastante. Comparado a antes, temos menos tempo, então cheguem cedo.
- Uso にかわって para dizer que alguém age no lugar de outra pessoa.
- Não confundo にかわって (em vez de) com に変わって (transformar-se em).
- Uso に比べて pondo o ponto de referência antes, ao contrário do português.
- Sei que に比べて é neutro e que より é a opção mais casual.

---

## Exercícios
### 1. (recognition) "No lugar dele, eu vou à reunião." Qual frase expressa a substituição de quem realiza a ação?
- **Resposta:** `{"choices": ["彼にかわって会議に出席します", "彼に変わって会議に出席します", "彼に比べて会議に出席します"], "correct": "彼にかわって会議に出席します"}`
- にかわって significa agir no lugar de alguém. に変わって (do verbo 変わる) significaria transformar-se nele, e に比べて é comparação. A pronúncia de にかわって e に変わって é idêntica; só o kanji distingue os dois sentidos, que são bem diferentes.


### 2. (particle_choice) "Comparado com o verão, o inverno é frio": 夏＿＿＿＿冬は寒い. Qual completa a comparação, marcando o verão como referência?
- **Resposta:** `{"choices": ["に比べて", "にかわって", "にとって"], "correct": "に比べて"}`
- に比べて marca o ponto de referência (o verão) e a frase diz que o inverno é frio em relação a ele. にかわって seria substituição e にとって seria perspectiva.


### 3. (cloze) Complete com a expressão de comparação para "comparado ao ano passado, a atuação está boa": 去年＿＿＿＿＿演技がうまい
- **Resposta:** `{"text": "に比べて", "full": "去年に比べて演技がうまい"}`
- O ano passado (去年) é o ponto de referência e leva に比べて; depois a frase diz que a atuação (演技) está boa. Lembre que a referência vem ANTES, ao contrário do português.


### 4. (matching) Associe cada kanji ao seu sentido central.
- **Resposta:** `{"pairs": [["利", "lucro / vantagem"], ["信", "confiança / fé"], ["任", "responsabilidade / confiar"], ["引", "puxar / atrair"]]}`
- 利 (arroz e faca, colher o ganho), 信 (pessoa fiel à sua palavra), 任 (pessoa a quem se confia uma tarefa), 引 (arco cuja corda se puxa). 引 aparece em 引退 (aposentadoria).


### 5. (production) Escreva "No lugar do rei, o príncipe faz o discurso" usando 王 (おう), にかわって, 王子 (おうじ) e 演説 (えんぜつ).
- **Resposta:** `{"text": "王にかわって王子が演説をする", "accept": ["王にかわって王子が演説をする", "おうにかわっておうじがえんぜつをする", "王にかわって王子が演説をします", "おうにかわっておうじがえんぜつをします"]}`
- 王にかわって abre a substituição (no lugar do rei), 王子が é quem age de fato, e 演説をする é a ação. にかわって marca exatamente a troca de quem realiza a ação.


### 6. (recognition) Qual destes kanji significa 'assunto, caso'?
- **Resposta:** `{"choices": ["件", "任", "信", "側"], "correct": "件"}`
- 件 (ケン) é o assunto ou caso de que se trata. Ele junta o radical de pessoa com 牛, e não deve ser confundido com 任 (encarregar) nem com 信 (confiar).


### 7. (recognition) Qual destes kanji se lê がわ e significa 'lado'?
- **Resposta:** `{"choices": ["側", "面", "方", "所"], "correct": "側"}`
- 側 (がわ) é o lado de algo, inclusive o lado de quem fala em uma discussão. 面 (めん) é face ou aspecto, 方 (ほう) é direção e 所 (ところ) é lugar.


### 8. (recognition) Qual destes kanji carrega a ideia de 'existir, estar presente'?
- **Resposta:** `{"choices": ["在", "場", "立", "住"], "correct": "在"}`
- 在 (ザイ) é existir ou estar situado em algum lugar, e o verbo ある às vezes aparece escrito 在る com esse kanji. 場 é o lugar em si, 立 é ficar de pé e 住 é morar.


### 9. (recognition) Qual destes kanji significa 'grupo, equipe'?
- **Resposta:** `{"choices": ["組", "集", "会", "族"], "correct": "組"}`
- 組 (くみ) é o grupo, a turma que se monta. O radical do fio, à esquerda, traz a ideia de coisas trançadas juntas. 集 é juntar, 会 é reunir e 族 é o clã, a família.


