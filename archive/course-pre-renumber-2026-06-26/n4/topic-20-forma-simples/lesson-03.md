# Restrição com しか〜ない: "só, apenas"

> Lição `les:n4-forma-simples-03` · tópico `top:n4-forma-simples` · **needs_review** (Layer C, aguarda professor).

**Objetivos:**
- Expressar limitação e escassez ('só / apenas') com しか〜ない
- Entender que しか sempre pede o verbo no negativo, mas o sentido é positivo-restritivo
- Aplicar o padrão a quantidades e números (千円しかない = 'só tenho mil ienes')
- Reconhecer e usar advérbios de quantidade como 殆ど e 大抵

**Introduz:** gramática [gp-100, gp-118] · vocabulário [壊す, 大抵, 必ず, 折る, 拾う, 止む, 殆ど, 沸く, 焼く, 盗む, 等々, 踏む] · kanji [地 新] · kana [—]

**Frases (por ID, do banco dissecado):** `sent:tatoeba-172845`, `sent:tatoeba-179727`, `sent:tatoeba-2464847`, `sent:tatoeba-4849`

---

### Restrição com しか〜ない: "só, apenas"
Já dá para dizer muita coisa em japonês, mas ainda falta um jeito natural de dizer "só isso, nada mais". É aí que entra o padrão gp-118. Ele expressa limitação e escassez: "só", "apenas", "nada além de". O detalhe que pega todo mundo de surpresa é que o verbo fica sempre no negativo, mas o sentido da frase é positivo e restrito. Pense assim: しか sozinho já diz "fora isso, não tem"; o negativo só completa esse "não tem".

#### A forma geral: 〜しか com o verbo no negativo
A receita é simples: primeiro a coisa limitada, depois しか, e por fim o verbo no negativo. Em português a gente diz "só resta esperar"; em japonês a estrutura literal é "fora esperar, não há":
Molde: [coisa] しか [verbo no negativo]
> 🗣 待つしかない。 — Só resta esperar.
Repare nas peças: 待つ ("esperar"), depois しか, e por fim ない ("não há"). O ない é o negativo, mas ninguém entende "não esperar"; entende-se "a única opção é esperar". Esse é o coração do padrão.

> **[l1-pitfall]**
Em português usamos "só" com verbo afirmativo ("eu só tenho isso"). Em japonês, com しか o verbo tem de ir para o negativo. Não traduza ao pé da letra: o negativo japonês vira afirmativo limitado em português.
> 🗣 今しかない。 — É agora ou nunca.
Aqui temos 今 ("agora"), depois しか, e ない, o que dá literalmente "fora agora, não há" e equivale ao nosso "é agora ou nunca". Note como しか normalmente substitui partículas como は, が ou を: a gente não diz 今がしか, só 今しか.

#### Com números e quantidades: "só mil ienes"
O mesmo recurso serve para quantidades, e isso é tão comum que tem até um nome próprio, gp-100. Você gruda しか〜ない num número ou medida para dizer que aquela quantia é pequena ou insuficiente:
> 🗣 あと1日しかない。 — Só resta um dia.
As peças aqui são あと ("restante"), 1日 ("um dia"), depois しか e ない: "só resta um dia". O tom é de pouco, de que o tempo está acabando.
> 🗣 金は少ししかない。 — Eu só tenho pouco dinheiro.
Nesta, 金 é "dinheiro", 少し é "um pouco", e しか〜ない reforça a escassez: "eu só tenho pouco dinheiro". O clássico 千円しかない segue a mesma lógica: "só tenho mil ienes (e é pouco)".

> **[tip]**
Guarde a fórmula: [quantidade ou coisa] しか [verbo no negativo]. Troque só o miolo e você cobre tempo (1日しかない), dinheiro (千円しかない) e ações (待つしかない).

#### Advérbios de quantidade que combinam
Para falar de "quanto" no dia a dia, vale conhecer alguns advérbios úteis. Eles ajudam a dizer com que frequência ou em que medida algo acontece:
- 殆ど: quase, praticamente
- 大抵: em geral, normalmente
- 必ず: sempre, sem falta
- 等々: etc., e assim por diante (serve para encerrar listas)
Aproveite também para fixar vários verbos do cotidiano, úteis para descrever ações comuns:
- 拾う: pegar do chão
- 盗む: roubar
- 折る: quebrar, partir
- 壊す: destruir
- 踏む: pisar
- 止む: cessar
- 沸く: ferver
- 焼く: queimar, assar

#### Dois kanji novos
地 significa "solo, terra". A leitura mais comum é ち (como em 地下, "subsolo"). Mnemônico: à esquerda fica o radical de terra (土), e à direita uma serpente rastejando pelo chão. Onde a cobra rasteja, é o solo.
新 significa "novo", e aparece no adjetivo 新しい ("novo"). Mnemônico: junta árvore (木) com um machado (斤) à direita; derrubar a árvore com o machado deixa a madeira nova em folha para construir.

> **[example]**
Juntando tudo: 新しい傘は千円しかなかった = "o guarda-chuva novo custou só mil ienes". Note o negativo no passado, なかった, mantendo o sentido de "foi só isso".

#### Hora de praticar

#### Leitura
> 📖 一人で行くしかない。2時間しかないんだ。あと1日しかない。やるしかない。 — Só resta ir sozinho. É que eu só tenho duas horas. Só resta um dia. Não temos escolha a não ser fazer.
- Sei dizer "só / apenas" com しか e o verbo no negativo.
- Aplico しか〜ない a números e quantidades (千円しかない).
- Lembro que しか pede o negativo, mas o sentido é positivo-limitado.
- Reconheço os kanji 地 ("solo") e 新 ("novo").

---

## Exercícios
### 1. (recognition) {'pt-BR': 'No padrão しか〜ない, em que forma fica SEMPRE o verbo?'}
- **Resposta:** `{"choices": ["no negativo", "no passado afirmativo", "na forma -te"], "correct": "no negativo"}`
- {'pt-BR': "しか exige o verbo no negativo (ない), mas o sentido geral da frase é positivo-limitado: 'só / apenas'. Ex.: 待つしかない = 'só resta esperar'."}


### 2. (cloze) {'pt-BR': "Complete com o negativo para dizer 'só resta esperar': 待つしか＿。"}
- **Resposta:** `{"text": "ない", "full": "待つしかない"}`
- {'pt-BR': "待つ ('esperar') + しか + ない ('não há'). O ない é o negativo obrigatório; a frase quer dizer 'a única opção é esperar'. Não traduza 'não esperar'; o negativo japonês vira afirmativo limitado em pt-BR."}


### 3. (particle_choice) {'pt-BR': "Escolha o que vai no lugar de ＿ para dizer 'só tenho mil ienes': 千円＿ない。"}
- **Resposta:** `{"choices": ["しか", "だけ", "から"], "correct": "しか"}`
- {'pt-BR': "しか combina com o negativo (ない) para marcar escassez: 千円しかない = 'só tenho mil ienes (e é pouco)'. だけ NÃO usa verbo negativo, então não cabe aqui."}


### 4. (sentence_build) {'pt-BR': "Monte 'Só resta um dia.' com as peças: [ない] [あと] [しか] [1日]"}
- **Resposta:** `{"order": ["あと", "1日", "しか", "ない"], "text": "あと1日しかない。"}`
- {'pt-BR': "あと ('restante') + 1日 ('um dia') + しか + ない. O padrão [quantidade] + しか + verbo negativo dá o tom de pouco: 'só resta um dia'."}


### 5. (production) {'pt-BR': "Diga em japonês 'É agora ou nunca.' (literalmente 'fora agora, não há') usando 今 e o padrão しか〜ない."}
- **Resposta:** `{"text": "今しかない。", "accept": ["今しかない", "いましかない", "今しかない。", "いましかない。"]}`
- {'pt-BR': "今 ('agora') + しか + ない. しか substitui partículas como は/が, então não se diz 今がしか; só 今しか. O negativo ない fecha o sentido 'é agora ou nunca'."}


