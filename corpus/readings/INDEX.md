...
Per box: `{slug, level, gated_to_lesson, title, jp, tokens:[{s,r,ro,pos}], translation:{pt-BR,en}, length_band, uses:{kanji,vocab}, source_slugs:[sent:…]}`. `uses.vocab` holds published `vocab:<jmdict_id>` slugs (never headwords — 93 headwords name more than one record). `source_slugs` credit the underlying bank sentences (provenance). Layer **B** (derived-and-verified), `needs_review: true`.
