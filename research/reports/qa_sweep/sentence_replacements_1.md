...
| Outcome | Slots |
|---|---|
| **R**, at least one fully compliant **real** candidate proposed (level ≤ lesson, load ≤ budget, correct teaching point) | 22 |
| **G**, only **AI-generated** compliant candidates exist (spec 1.2 preference for selection cannot be met here) | 6 |
| **N**, **no compliant candidate of any kind**; needs authoring or an upstream unlock fix | 26 |
| **L**, **no replacement needed**: load is already ≤ budget and only the sentence's own level grade fails | 19 |
| **X**, recommend **removal** rather than replacement (mis-teaches, near-duplicate, or off-register) | 6 |
| **Total** | **79** |

The six **X** slots: `sent:tatoeba-174533` (n5-verbos-02), `sent:tatoeba-221717` and `sent:tatoeba-994752`
(n4-passiva-02), `sent:tatoeba-85538` (n5-desu-wa-04 **and** n5-conectando-03, two slots),
`sent:tatoeba-106462` (n4-suposicao-01).

### Defect class per slot (a slot can carry more than one)

Counted mechanically over the 79 queue rows unless marked "judged".

| Class | Slots |
|---|---|
| Over the i+1 budget (`load > budget`) | 44 |
| Graded above the lesson level | 59 |
| Load inflated by a wrong vocab link (A1 `ほう→報`, A2 `よう→用`) | 8 |
| Load inflated by a late or never-issued unlock of an elementary item (A3, A4) | 30 |
| Load inflated by A1/A2/A3/A4 in any combination (union of the two rows above) | 31 |
| Teaching point whose entire tagged pool in `bank.json` is **AI-generated**, zero real sentences | 13 |
| Sentence carries grammar the lesson has not taught (judged) | 4 |
| Sentence is **mis-tagged** for the grammar point it is filed under (judged) | 3 |
| Register or content mismatch with the lesson and its audience (judged) | 4 |

Grammar keys in my slice with **zero real sentences** in the whole bank (real / AI counts):
`o-go` 0/5, `gp-90` 0/5, `gp-102` 0/5, `gp-103` 0/5, `gp-104` 0/5, `gp-107` 0/4, `gp-111` 0/5,
`gp-112` 0/5, `kana` 0/5, `mitai-na` 0/5, `ni-ki-ga-tsuku` 0/5. Eleven points where the corpus violates the
spec's "prefer SELECTION over GENERATION" rule outright, not by a thin margin.

The 3 mis-tagged slots: `sent:tatoeba-85538` (tagged `demo`, but its でも is copula-て + binding も; two
slots) and `sent:tatoeba-123214` (tagged `n3-you-ni-shimashou`, but it is どのように + しましょう).

The 4 grammar-ahead-of-level slots: `sent:tatoeba-174533` (命令形 閉めろ, n5-verbos-02),
`sent:tatoeba-8608115` (volitional 話そう plus the N4 `ato-de`, n5-passado-03), `sent:gen-867d5c2e8dc3`
(tagged `gp-64`, the N4 transitive/intransitive contrast, in n5-verbos-02), `sent:tatoeba-122195`
(〜てくる, which the lesson body itself flags as beyond level, n5-verbos-03).

The 4 register/content mismatches: `sent:tatoeba-174533` (blunt imperative in the second verb lesson),
`sent:tatoeba-182469` (polite ました in a lesson about casual speech), `sent:tatoeba-994752`
(幽霊 "fantasmas", N1 kanji, in an N4 keigo lesson), `sent:tatoeba-106462` ("Dizem que ele morreu", a
near-duplicate of `-104331` with no pedagogical gain).

### Mis-tagged **candidates** a load-ranked pick would wrongly select

Recorded so the next reviewer does not take them off a sorted list:

| candidate | tagged | actually |
|---|---|---|
| `sent:tatoeba-148197` 秋はいつしか冬となった。 | `shi` | いつしか is one adverb; no し particle in the sentence |
| `sent:tatoeba-229897` ある外国人が私に駅がどこにあるかと聞いた。 | `to-kiita` ("ouvi dizer que") | と聞いた here means "perguntou" |
| `sent:tatoeba-81309` 毎日どのようにして学校へ行くのですか。 | `you-ni-you-na` | interrogative どのように, not purpose ように |
| `sent:tatoeba-8687007` / `-8719362` / `-3488181` / `-205726` (それでいい？ / それで十分？ / それでいいよ。 / それで十分だよ。) | `gp-134` (causal それで) | それで = "isso basta", not the causal connector |
| `sent:tatoeba-213768` そう水くさくするな。 | `ku-suru` | 水くさい is the idiom "ser distante"; does not show "deixar X mais ~" |
| `sent:tatoeba-213828` そうとはまず思えない。 | `gp-66` (まず = "primeiro") | まず here means "dificilmente" |

### Systemic findings (Part A)

| # | Finding | Bank-wide scope | In my slice |
|---|---|---|---|
| A1 | `ほう` of `ほうがいい` linked to 報 (`vocab:1515620`) instead of 方 (`vocab:1516930`); token gloss contradicts the linked headword | 39 sentences | 4 sentences, 3 queue slots |
| A2 | `よう` of `ように` linked to 用 (`vocab:1546200`) instead of 様 (`vocab:1605840`); same gloss/headword contradiction | 127 sentences | 9 sentences, 5 queue slots |
...
| A1–A4 combined | queue rows whose `load` is inflated by at least one of the four | | **31 of 79 slots** |
